...
and adds a reserialization sensitivity control and return-type/resource cases,
so its 20 cases are not a claim
...
failures to canonicalization_failure, and rejects non-Buffer projection returns
before the canonical-storage branch or hashing. Projection checkpoint exceptions retain
their original identity, including non-Error thrown values; typed StoredInputError
also retains its owner in both projection and digest catches. Checkpoints before and after the digest remain outside
the new catch. Resource enforcement by the outer supervisor still takes precedence.
Every checkpoint reached by the small stored-byte fixture is tested for propagation.
A separate >2 KB fixture tests periodic skip/quote/valueEnd checkpoints with typed
budget errors, Error cancellation and thrown undefined. Projection must never
catch checkpoint exceptions. The existing canonicalizer catch and
...
These receipts describe isolated test copies, not the ordinary candidate lane.
The three baseline copies are unmodified and share the candidate inventory hash;
consumers must also inspect the lane, case identity and MUTATION.json, rather than
classifying receipts by manifest hash alone. Other copies are deliberately modified.
Repinning a test copy is not a release or trust grant.
...
| Projection returns undefined / string × valid / K-fail | 4 | canonicalization_failure, no report/reference/forward |
| Digest throws typed resource limit × valid | 1 | resource_limit, no report/reference/forward |
...
The host archive has 190 existing checks plus 21 separately identified copy checks
(211 planned), not 211 ordinary calls against one runtime inventory. Its nested
helper-faults/RESULTS.json has 21 cases; ordinary candidate calls remain 163.
...
Use Node 24.19.0 / Python 3.12.14 for real-worker tests. Author local run: 245
component tests, zero skips; 20/20 inner helper cases. Real local cgroup execution
is NOT_RUN (read-only mount). Exact-head hosted results, archive digest and timing
are recorded in the PR body after CI; the planned 211 count alone is not evidence.
...
### Review received and repair disposition

The user supplied an implementation review of `fa5cefa6681b14736ec8f2f6617e44415f0d9caa`
in the conversation on 2026-09-18. No repository review commit/blob was provided;
this is an author disposition summary, not byte-preserved reviewer intake.
The reviewer continues the same session/context as the PR #22 review and explicitly
does **not** qualify as an independent investigator for the RFC Research Gate.
They report direct reading of Protocol D0 `1eb6b93`. Their implementation findings
and PR #22 repair confirmation must not be promoted to independent clearance.

The substantive finding was REPAIR_REQUIRED: non-Buffer projection returns escaped
the throw-only repair. Author reproduction on fa5cefa with the extended harness
matched all four reported outcomes: undefined/canonical and string/canonical
became internal_error, undefined/noncanonical refused, string/noncanonical emitted
a report with a wrong reference digest. The added typed digest-resource case also
demonstrated misclassification. Before repair, 16/20 assertions passed; these
results are separate from the original retained 8/14 evidence.

Repair: a Buffer guard before canonicalStorage, four return-type fault cases,
typed digest exception passthrough with one injected resource case, periodic
checkpoint propagation tests, the no-catch comment and baseline receipt caveat.
No RangeError-to-resource policy is inferred: only an already typed StoredInputError
retains its classification. PR #22's five repair items were confirmed by the supplied
review; its target remains `735e620`. Close-only implementation confirmation of this
repair is still requested, with the same independence limitation recorded.
...
2. Do mutations actually reach the intended helper, and are assertions against
   owned refusal semantics rather than generated candidate expectations? Check
   both thrown failures and non-Buffer returns on canonical/noncanonical Records.
