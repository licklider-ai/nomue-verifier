/**
 * Phase 2A successor bundle pipeline (urn:nomue:bundle:itgc-guarantee:0.2.1-draft.1).
 *
...
const CHECK_VERSION_2A = "0.2.0-draft.1";
const CHECK_VERSION_021 = "0.2.1-draft.1";
...
export function runPhase2a021Bundle(
  parsed: unknown,
  ...
  if (conformance.outcome !== "pass") {
    ...
    results.push(
      notRun(CHECK_IDS_2A.integrity, CHECK_VERSION_2A, scope, codes),
      notRun(CHECK_IDS_2A.admissibility, CHECK_VERSION_2A, scope, codes),
      notRun(CHECK_IDS_2A_021.computability, CHECK_VERSION_021, scope, codes),
      notRun(CHECK_IDS_2A_021.welch, CHECK_VERSION_021, scope, codes),
    );
  } else {
    ...
    results.push({
      ...
      check_version: CHECK_VERSION_2A,
      execution: "completed",
    ...
    if (admissibilityResult.outcome !== "pass") {
      ...
      results.push(
        notRun(
          CHECK_IDS_2A_021.computability,
          CHECK_VERSION_021,
          analysisScope,
          admissibilityCodes,
        ),
        notRun(CHECK_IDS_2A_021.welch, CHECK_VERSION_021, resultScope, admissibilityCodes),
      );
    } else {
      ...
      try {
        ...
        if (isNumericalPUnderflow(welch.p_value, welch.test_statistic, welch.degrees_of_freedom)) {
          computabilityResult = {
            check_id: CHECK_IDS_2A_021.computability,
            check_version: CHECK_VERSION_021,
            execution: "completed",
            outcome: "fail",
            scope: analysisScope,
            reason_codes: ["NRS-P-VALUE-UNDERFLOW", "NRS-NUMERICAL-COMPUTABILITY-FAILED"],
          };
        } else if (welch.p_value <= 0 || welch.p_value > 1) {
          computabilityResult = {
            check_id: CHECK_IDS_2A_021.computability,
            check_version: CHECK_VERSION_021,
            execution: "completed",
            outcome: "fail",
            scope: analysisScope,
            reason_codes: ["NRS-NON-FINITE-NUMERIC-VALUE", "NRS-NUMERICAL-COMPUTABILITY-FAILED"],
          };
        } else {
          computabilityResult = {
            check_id: CHECK_IDS_2A_021.computability,
      ...
      if (computabilityResult.outcome !== "pass" || welch === null) {
        results.push(
          notRun(
            CHECK_IDS_2A_021.welch,
            CHECK_VERSION_021,
            resultScope,
            computabilityResult.reason_codes,
          ),
        );
      } else {
        const welchCheck = resources.checks.get(CHECK_IDS_2A_021.welch);
        ...
        welch.group_summaries.forEach((summary, index) => {
          ...
          if (!withinNumericTolerance(summary.mean, declaredSummary.mean, tol("mean"))) {
            mismatches.push({
              quantity: "mean",
              group_id: summary.group_id,
        ...
        for (const [name, declaredValue, recomputedValue, specificCode] of scalarComparisons) {
          if (name === "p_value") {
            const pTol = tol("p_value");
            if (!withinPValueTolerance021(recomputedValue, declaredValue, pTol.relative)) {
              mismatches.push({
                quantity: name,
          ...
          } else if (!withinNumericTolerance(recomputedValue, declaredValue, tol(name))) {
            mismatches.push({
              quantity: name,
              declared: declaredValue,
              recomputed: recomputedValue,
        ...
        results.push({
          check_id: CHECK_IDS_2A_021.welch,
          check_version: CHECK_VERSION_021,
          execution: "completed",
